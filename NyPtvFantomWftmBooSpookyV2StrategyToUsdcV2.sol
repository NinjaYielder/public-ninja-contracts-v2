 ...
 * out quoting reference 'NyPtvFantomWftmBooSpookyV2StrategyToUsdcV2'
 *
...
interface IMasterChef {
  ...
  function pendingBOO(uint256 _pid, address _to)
    external
    view
...
contract NyPtvFantomWftmBooSpookyV2StrategyToUsdcV2 is
  NYBaseProfitTakingStrategyV2
{
  ...
  address public constant SPOOKY_SWAP_FARM_V2 =
  ...
  address public constant LP_TOKEN_0 =
    address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
  address public constant LP_TOKEN_1 =
    address(0x841FAD6EAe12c286d1Fd18d1d525DFfA75C7EFFE);
  ...
  function initialize(
    ...
    __NYBaseProfitTakingStrategy_init(
      _vault,
      _treasury,
      _strategistRemitter,
      _strategists,
  ...
  function _deposit() internal override {
    uint256 underlyingBalance = IERC20Upgradeable(LP_TOKEN).balanceOf(
      address(this)
    );
    if (underlyingBalance != 0) {
      IERC20Upgradeable(LP_TOKEN).safeIncreaseAllowance(
        SPOOKY_SWAP_FARM_V2,
        underlyingBalance
      );
      IMasterChef(SPOOKY_SWAP_FARM_V2).deposit(POOL_ID, underlyingBalance);
    }
  }
  ...
  function _withdraw(uint256 _amount) internal override {
    ...
    if (underlyingBalance < _amount) {
      IMasterChef(SPOOKY_SWAP_FARM_V2).withdraw(
        POOL_ID,
        _amount - underlyingBalance
      );
    }
  ...
  function _swapFarmEmissionTokens() internal {
    ...
    uint256[] memory amounts = IUniswapV2Router02(SPOOKY_ROUTER).getAmountsOut(
      booBalance,
      booToUsdcPath
    );
  ...
  function balanceOf() public view override returns (uint256) {
    (uint256 amount, ) = IMasterChef(SPOOKY_SWAP_FARM_V2).userInfo(
      POOL_ID,
      address(this)
    );
  ...
  function _reclaimUnderlying() internal override {
    IMasterChef(SPOOKY_SWAP_FARM_V2).emergencyWithdraw(POOL_ID, address(this));
  }
  ...
  function estimateHarvest()
    ...
    returns (uint256 profit, uint256 callFeeToUser)
  {
    uint256 pendingReward = IMasterChef(SPOOKY_SWAP_FARM_V2).pendingBOO(
      POOL_ID,
      address(this)
    );
  ...
  function _retireStrategy() internal override {
    ...
    (uint256 poolBal, ) = IMasterChef(SPOOKY_SWAP_FARM_V2).userInfo(
      POOL_ID,
      address(this)
    );
    if (poolBal != 0) {
      IMasterChef(SPOOKY_SWAP_FARM_V2).withdraw(POOL_ID, poolBal);
    }
