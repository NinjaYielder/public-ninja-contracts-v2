...
// No permissions granted before Thursday, 6 Jun 2024, then GPL-3.0 after this date.
 ...
 * out quoting reference 'NyPtvFantomWftmBooSpookyV2VaultToUsdcV2'
 *
...
error NYProfitTakingVault__CannotWithdrawProfitToken();

error NYProfitTakingVault__CannotWithdrawUnderlying();

error NYProfitTakingVault__InitializationPeriodPassed();

error NYProfitTakingVault__NotInitialized();

error NYProfitTakingVault__OnlyStrategy();

error NYProfitTakingVault__ProfitTimeOutOfBounds();

error NYProfitTakingVault__TermsAlreadyAccepted();

error NYProfitTakingVault__TransferNotAllowed();

error NYProfitTakingVault__UnderlyingCapReached(uint256 underlyingCap);

error NYProfitTakingVault__ZeroAmount();

contract NYProfitTakingVaultBaseV2 is ERC20, Ownable, ReentrancyGuard {
  using SafeERC20 for IERC20;

  address public strategy;

  uint256 public constant PERCENT_DIVISOR = 10000;
  uint256 public constant PROFIT_TOKEN_PER_SHARE_PRECISION = 1e24;
  uint256 public constant INITIALIZATION_WINDOW = 1200;
  uint256 public immutable UNDERLYING_DECIMALS;
  uint256 public underlyingCap;

  uint256 public lastProfitTime;
  ...
  event ProfitReceivedFromStrategy(uint256 profitAmount);
  event TermsAccepted(address user);
  event UnderlyingCapUpdated(uint256 newUnderlyingCap);
  event Withdraw(address indexed user, uint256 shares, uint256 profitAmount);
  ...
  constructor(
    ...
    UNDERLYING_DECIMALS = IERC20Decimals(_underlying).decimals();
  }
  ...
  function balance() public view returns (uint256) {
    return
      underlying.balanceOf(address(this)) + IStrategy(strategy).balanceOf();
  }
  ...
  function deposit(uint256 _amount) public nonReentrant {
    if (!initialized) {
      revert NYProfitTakingVault__NotInitialized();
    }
    if (_amount == 0) {
    ...
    user.amount += _amount;
    user.rewardDebt =
      (userAmount * accProfitTokenPerShare) /
      PROFIT_TOKEN_PER_SHARE_PRECISION;

    if (pending > 0) {
      profitToken.safeTransfer(msg.sender, pending);
    }
  ...
  function depositProfitTokenForUsers(uint256 _amount) external nonReentrant {
    ...
    accProfitTokenPerShare += ((_amount * PROFIT_TOKEN_PER_SHARE_PRECISION) /
      totalShares);
    lastProfitTime = block.timestamp;
  ...
  function inCaseTokensGetStuck(address _token) external onlyOwner {
    ...
    if (_token == address(profitToken)) {
      revert NYProfitTakingVault__CannotWithdrawProfitToken();
    }
  ...
  function getUserPendingEarnings(address _user)
    internal
    view
    returns (uint256)
  {
    uint256 userAmount = balanceOf(_user);
    if (userAmount == 0) {
      return 0;
    }
    if (accProfitTokenPerShare == 0) {
      return 0;
    }

    UserInfo storage user = userInfo[_user];
    uint256 pending = (userAmount * accProfitTokenPerShare) /
      PROFIT_TOKEN_PER_SHARE_PRECISION -
      user.rewardDebt;
  ...
  function transfer(address, uint256) public pure override returns (bool) {
    revert NYProfitTakingVault__TransferNotAllowed();
  }
...
contract NyPtvFantomWftmBooSpookyV2VaultToUsdcV2 is NYProfitTakingVaultBaseV2 {
  constructor(
    address _token,
    address _profitToken,
    string memory _name,
